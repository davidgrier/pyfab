...
class ModifyUltraTrap(task):
    """Delay, record, and move traps in the z direction 
    by changing their hologram."""
    ...
    def initialize(self, frame):
        ...
        for t in self.traps.flatten():
            if isinstance(t, QUltraTrap):
                self.trap = t  # selecting the ultra traps
        self.DdeltaZ = -1
        self.FirstDeltaZ = 10
        self.LastDeltaZ = 0
    ...
    def dotask(self):
        ...
        if isinstance(self.trap, QUltraTrap):
            ...
            self.register('Modify', group=self.traps,
                          NewDeltaZ=self.FirstDeltaZ, NewDeltaPhi=0)
